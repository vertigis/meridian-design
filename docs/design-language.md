...
Use the theme palette rather than one-off colour values (see [Colors](#colors)), and verify unfamiliar pairings with a contrast checker before committing to them.
...
-   **Colour.** The primary and secondary theme colours are the loudest tool available, which is why they are reserved for the action that matters and for status that needs to be noticed. Greys carry structure. See [Colors](#colors).
